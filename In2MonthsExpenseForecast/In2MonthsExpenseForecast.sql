...
WHERE TRANSCODE != "Transfer"
    AND c.Status != "V"
    AND Moment >= date("now", "localtime","start of month", "+2 month") 
...
CASE 
    WHEN strftime("%d", c.TRANSDATE) >= strftime("%d", date("now", "localtime")) THEN date(c.TRANSDATE, "localtime", "+2 month")
...
FROM BILLSDEPOSITS_V1 c
    ...
    LEFT JOIN SUBCATEGORY_V1 cat2 ON cat2.SUBCATEGID = c.SUBCATEGID
WHERE TRANSCODE != "Transfer"
    AND c.TRANSDATE <= date("now", "localtime","start of month", "+2 month", "-1 day")
